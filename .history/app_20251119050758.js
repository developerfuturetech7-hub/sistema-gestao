...
class SistemaGestao {
    ...
    init() {
        ...
        setTimeout(() => {
            ...
            // Proteger o campo contra modificações indesejadas
            const campoSaldo = document.getElementById('saldoInicial');
            const observer = new MutationObserver(() => {
                if (campoSaldo.value === '' || campoSaldo.value === '0') {
                    if (this.dados.saldoInicial && this.dados.saldoInicial > 0) {
                        campoSaldo.value = this.dados.saldoInicial;
                        console.log('Campo foi resetado, restaurando:', this.dados.saldoInicial);
                    }
                }
            });
            observer.observe(campoSaldo, { attributes: true, attributeFilter: ['value'] });
        }, 100);
    }

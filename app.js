...
class SistemaGestao {
    ...
    init() {
        ...
        setTimeout(() => {
            ...
            if (this.dados.saldoInicial && campoSaldo) {
                campoSaldo.value = this.dados.saldoInicial;
            }
            
            // Proteger o campo com Object.defineProperty
            if (campoSaldo) {
                const self = this;
                let valorProtegido = this.dados.saldoInicial || 0;
                
                Object.defineProperty(campoSaldo, 'value', {
                    get: function() {
                        return valorProtegido;
                    },
                    set: function(novoValor) {
                        valorProtegido = novoValor;
                        campoSaldo.setAttribute('value', novoValor);
                        // Salvar quando usuário digitar
                        if (novoValor && parseFloat(novoValor) !== self.dados.saldoInicial) {
                            self.dados.saldoInicial = parseFloat(novoValor) || 0;
                            self.salvarDados();
                        }
                    }
                });
                
                // Garantir que sempre mostre o valor salvo
                campoSaldo.value = this.dados.saldoInicial;
            }
        }, 100);
    }
    ...
    carregarDados() {
        ...
        if (dadosSalvos) {
            ...
            if (this.dados.saldoInicial === undefined || this.dados.saldoInicial === null) {
                ...
                const saldoBackup = localStorage.getItem('saldoInicial');
                this.dados.saldoInicial = saldoBackup ? parseFloat(saldoBackup) : 0;
            }
        ...
        const campoSaldo = document.getElementById('saldoInicial');
        if (campoSaldo) {
            // Tentar carregar do dados ou do backup
            const saldoBackup = localStorage.getItem('saldoInicial');
            const saldoFinal = this.dados.saldoInicial || (saldoBackup ? parseFloat(saldoBackup) : 0);
            
            if (saldoFinal !== undefined && saldoFinal !== null) {
                campoSaldo.value = saldoFinal;
                this.dados.saldoInicial = saldoFinal;
                console.log('Saldo inicial carregado:', saldoFinal);
            } else {
                campoSaldo.value = 0;
    ...
    salvarDados() {
        ...
        // Salvar saldo em chave separada como backup
        localStorage.setItem('saldoInicial', this.dados.saldoInicial);
    }
    ...
    mostrarPagina(nomePagina) {
        ...
        switch(nomePagina) {
            ...
            case 'financeiro':
                ...
                // FORÇAR restauração do saldo ao mostrar página financeiro
                setTimeout(() => {
                    const campoSaldo = document.getElementById('saldoInicial');
                    const saldoBackup = localStorage.getItem('saldoInicial');
                    const saldoFinal = this.dados.saldoInicial || (saldoBackup ? parseFloat(saldoBackup) : 0);
                    if (campoSaldo && saldoFinal) {
                        campoSaldo.value = saldoFinal;
                        console.log('✅ Saldo restaurado ao mostrar Financeiro:', saldoFinal);
                    }
                }, 50);
                break;
            case 'orcamentos':
